        ...
        if (rejectedFiles.length > 0) {
          for (let i = 0; i < rejectedFiles.length; i++) {
            ...
            if (rejectedFiles[i].errors[0]?.code === "file-invalid-type") {
              toast.error("Only PNG, JPEG, JPG, WEBP files are allowed");
              break;
            }
    ...
    return (
      ...
      >
        <div
          ...
          className={cn("grid w-full gap-2 overflow-hidden focus:outline-none", className)}
          dir={dir}
          {...props}
        >
          {children}
...
>(({ className, index, children, ...props }, ref) => {
  ...
  return (
    <div
      ref={ref}
      className={cn(
        "relative flex cursor-default items-center rounded-lg px-4",
        className,
        isSelected ? "bg-muted" : "",
      )}
      {...props}
    >
      <div className="flex h-full w-full items-center gap-1.5 text-sm font-medium leading-none tracking-tight">
        {children}
      </div>
  ...
  ({ className, children, ...props }, ref) => {
    ...
    return (
      ...
      >
        <div
          className={cn(
            `w-full rounded-lg border border-dashed border-muted-foreground/50 duration-300 ease-in-out`,
            dropzoneState.isDragActive && "border-primary bg-muted",
            (dropzoneState.isDragReject || isFileTooBig) && "border-red-500",
            className,
          )}
          {...rootProps}
        >
          <div className="flex h-32 w-full flex-col items-center justify-center space-y-2 py-4 text-sm text-muted-foreground">
            {dropzoneState.isDragActive ? <p>Drop the files here ...</p> : <>{children}</>}
          </div>
        </div>
